...
const LegalRepresentativeModal = ({ show, handleClose }: any) => {
    type Member = {
    name: string;
    ownerType: string;
    ownership: string;
  ...
  { name: "", ownerType: "", ownership: "" , proof: "",}
]);
 ...
 const handleChange = (
  ...
  const addRow = () => {
    setMembers([...members, { name: "", ownerType: "", ownership: "", proof: "", }]);
  };
  ...
  return (
    <Modal show={show} onHide={handleClose} centered className="common-modal lg-modal pt-lg-4 pt-5">
        ...
        <Modal.Header>
            <Modal.Title>
                <h5 className='m-0'>Legal Representative </h5>
            </Modal.Title>
        </Modal.Header>
        <Modal.Body className="common-modal-body">
            {members.map((member, index) => (
            <div className='member-list-add-card Legal-list-add-card' key={index}>
                <div className='row align-items-center pe-60 position-relative'>
                    <div className='col'>
                        <FloatingLabel controlId="floatingInput" label="Employee Full Name" className="mb-3">
                    ...
                    <div className='col'>
                        <FloatingLabel controlId="floatingInput" label="Position / Job Title" className="mb-3 position-relative">
                    ...
                    <div className='col'>
                        <div className='mb-3'>
                            <FloatingLabel controlId="floatingSelect" label="Identification Proof">
                            <Form.Select aria-label="Identification Proof"  value={member.ownerType} onChange={(e) => handleChange(index, "ownerType", e.target.value) }>
                                <option>Passport</option>
                                <option value="1">Passport 1</option>
                                <option value="2">Passport 2</option>
                                <option value="3">Passport 3</option>
                            </Form.Select>
                            </FloatingLabel>
                        </div>
                    </div>
                    <div className='col'>
                        <FloatingLabel controlId="floatingInput" label="Proof Id No." className="mb-3 position-relative">
                            <Form.Control type="text"  placeholder="Proof Id No."  value={member.proof} onChange={(e) => handleChange(index, "proof", e.target.value)}/>
                        </FloatingLabel>
                    </div>
...
export default LegalRepresentativeModal